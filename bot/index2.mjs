...
const explicitOrigin = process.env.PUBLIC_CARD_ORIGIN?.trim();
const PUBLIC_ORIGIN = explicitOrigin
  ? explicitOrigin.replace(/\/$/, '')
  : railwayDomain
    ? `https://${railwayDomain}`
    : null;
const CARD_IMAGE_URL = PUBLIC_ORIGIN ? `${PUBLIC_ORIGIN}/card.jpg?v=18` : null;
...
async function uploadRenderedPhoto(chatId) {
...
async function handleInlineQuery(query) {
  ...
  if (CARD_IMAGE_URL) {
    try {
      await bot('answerInlineQuery', {
        ...
        results: [{
          type: 'photo',
          id: 'pumb-public-photo-v18',
          photo_url: CARD_IMAGE_URL,
          thumbnail_url: CARD_IMAGE_URL,
          photo_width: 1200,
          photo_height: 630,
      ...
      console.log(`Inline public photo ready: ${CARD_IMAGE_URL}`);
      return;
    } catch (error) {
      console.error('Inline public photo failed:', error.message);
    }
  }
  ...
  if (cachedPhotoFileId) {
    try {
      ...
      console.log('Inline cached photo ready');
      return;
    } catch (error) {
      console.error('Inline cached photo failed:', error.message);
      cachedPhotoFileId = null;
    }
  }
  ...
  await bot('answerInlineQuery', {
    ...
    results: [],
    button: {
      text: 'Сначала откройте PUMB Card',
      start_parameter: 'card',
    },
  });
}
...
async function pollingLoop() {
  ...
  console.log(`Public origin: ${PUBLIC_ORIGIN ?? '[none]'}`);
  console.log(`Inline image URL: ${CARD_IMAGE_URL ?? '[cached-photo fallback]'}`);
...
server.listen(port, () => console.log(`Health/image server on :${port}`));
